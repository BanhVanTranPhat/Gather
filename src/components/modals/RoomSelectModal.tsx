...
export interface ServerRoom {
  ...
  /** Chỉ admin phòng (creator hoặc role admin) mới có thể sửa tên / xóa phòng */
  canManage?: boolean;
}
...
export default function RoomSelectModal({
  ...
  const handleDeleteRoom = async (room: ServerRoom) => {
    if (!room.canManage) return;
    if (!window.confirm(`Bạn có chắc muốn xóa phòng "${room.name}"? Hành động không thể hoàn tác.`)) return;
    setDeletingId(room.roomId);
    try {
      const res = await authFetch(`${serverUrl}/api/spaces/${encodeURIComponent(room.roomId)}`, {
        method: "DELETE",
      });
          ...
          {mode === "select" && (
            <>
              <p className="text-slate-400 text-sm mb-3">
                Chọn phòng để vào. Chỉ admin phòng mới có thể sửa tên hoặc xóa phòng.
              </p>
              {loading ? (
